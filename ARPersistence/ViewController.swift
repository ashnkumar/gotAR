...
//            snapshotThumbnail.isHidden = false
    ...
    @IBAction func removeAnObject(_ sender: Any) {
        print("wants to remove an object")
        sceneView.scene.rootNode.enumerateChildNodes { (node, stop) in
//            print(node)
            if let possNodeName = node.name {
                
                
                if titleCount == 0 {
                    if (possNodeName == "textNumberOne") {
                        ...
                        //                    SCNTransaction setAn
                        node.position = SCNVector3Make(0.0, 50.0, 0.0);
                        node.opacity = 0.0;
                        ...
                        sceneView.scene.rootNode.enumerateChildNodes { (node2, stop) in
                             if let possNodeName2 = node2.name {
                                print("IN SECOND ENUMERATION< NODE NAME")
                                print(possNodeName2)
                                if (possNodeName2 == "textNumberTwo") {
                                    print("YES WE HAVE TEXT 2")
                    ...
                    if (possNodeName == "textNumberTwo") {
                        ...
                        sceneView.scene.rootNode.enumerateChildNodes { (node2, stop) in
                            if let possNodeName2 = node2.name {
                                if (possNodeName2 == "textNumberThree") {
                                    SCNTransaction.animationDuration = 0.5
                                    node2.isHidden = false
                                    SCNTransaction.commit()
                                }
...
//            print(node)
//            if let possNodeName = node.name {
//                if (possNodeName == "textNumberTwo" || possNodeName == "textNumberThree") {
//                    print("HERE WE ARE!")
//                    node.isHidden = true
//                }
//            }
        ...
        do {
            if let fileURL = Bundle.main.path(forResource: "gottitle3", ofType: "m4a") {
...
//        }
//        guard let sceneURL = Bundle.main.url(forResource: "throneme", withExtension: "scn", subdirectory: "Assets.scnassets/throne"),
//            let referenceNode = SCNReferenceNode(url: sceneURL) else {
//                fatalError("can't load virtual object")
//        }
//        guard let sceneURL = Bundle.main.url(forResource: "got8", withExtension: "dae", subdirectory: "Assets.scnassets/throne"),
//            let referenceNode = SCNReferenceNode(url: sceneURL) else {
//                fatalError("can't load virtual object")
//        }
        guard let sceneURL = Bundle.main.url(forResource: "MainTitleScene", withExtension: "scn", subdirectory: "Assets.scnassets/throne"),
            let referenceNode = SCNReferenceNode(url: sceneURL) else {
                fatalError("can't load virtual object")
        }
